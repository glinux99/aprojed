...
<template>
    <AppLayout>
                                            ...
                                            <Column field="name" header="Collaborateur" sortable style="min-width: 22rem">
                                                <template #body="{ data }">
                                                    <div class="flex items-center gap-5 group cursor-pointer" @click="openEdit(data)">
                                                        <div class="relative">
                                                            <Avatar :image="data.profile_photo_url || data.avatar_url || null" :label="data.profile_photo_url || data.avatar_url ? '' : data.name?.charAt(0) || 'U'" shape="circle" size="xlarge"
                                                                class="shadow-lg" :class="{'bg-slate-200 text-slate-700': !(data.profile_photo_url || data.avatar_url)}" />
                    ...
                    <TabPanel header="Identité & Accès">
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6 px-4">
                            <div class="lg:col-span-4 flex flex-col items-center border-r border-slate-100 pr-6">
                                ...
                                <div v-if="currentUser.profile_photo_url" class="relative group w-40 h-40 rounded-[2rem] overflow-hidden border-4 border-white shadow-xl mb-4">
       ...
       <Dialog
    v-model:visible="viewUserSidebar"
    modal
    position="right"
    :header="false"
    :closable="false"
...
>
    <div class="px-8 py-5 bg-slate-900 text-white rounded-xl flex justify-between items-center relative z-50">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h4 class="font-black text-slate-100 m-0">Dossier Collaborateur</h4>
                <p class="text-[10px] uppercase tracking-widest text-slate-400">ID: #{{ currentUser.id }}</p>
            </div>
        </div>
        <Button icon="pi pi-times" variant="text" severity="secondary" rounded @click="viewUserSidebar = false" class="text-white hover:bg-white/10" />
    </div>

    <div v-if="currentUser.id" class="p-4 mt-4">
        <div class="grid grid-cols-12 gap-10">
            <!-- Colonne Gauche : Identité Visuelle -->
            <div class="col-span-12 md:col-span-4">
                <div class="sticky top-0">
                    <div class="relative group bg-white rounded-[2.5rem] p-3 border border-slate-200 shadow-2xl">
                        <div class="relative w-full aspect-square overflow-hidden rounded-[2.5rem] bg-slate-100 shadow-2xl border-4 border-white">
                            <div class="absolute inset-0 bg-cover bg-center bg-no-repeat"
                                :style="{ backgroundImage: currentUser.profile_photo_url ? `url(${currentUser.profile_photo_url})` : 'none' }">
                                <div v-if="!currentUser.profile_photo_url" class="w-full h-full flex items-center justify-center">
                        ...
                        <div class="py-6 text-center">
                            ...
                            <span :class="`text-[10px] font-black uppercase tracking-[0.3em] px-4 py-1.5 rounded-full bg-${currentUser.team?.color || 'slate'}-50 text-${currentUser.team?.color || 'slate'}-600 border border-${currentUser.team?.color || 'slate'}-100`">
                                {{ currentUser.team?.name || 'Sans département' }}
                            </span>
                        </div>
                    </div>

                    <div class="mt-6 p-6 bg-indigo-50/50 rounded-3xl border border-indigo-100">
                        <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-3">Taux Horaire</p>
                        <div class="flex items-baseline gap-2">
                            <span class="text-3xl font-black text-indigo-900">{{ currentUser.hourly_rate || 0 }}</span>
                            <span class="text-sm font-bold text-indigo-600 uppercase">EUR / Heure</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-12 md:col-span-8 space-y-8">
                <div>
                    <span class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 block">Informations Générales</span>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase">Email Professionnel</label>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <span class="font-bold text-slate-700 font-mono">{{ currentUser.phone || '-' }}</span>
                        </div>
                    </div>
                </div>

                <div>
                    <span class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 block">Affectation & Contrat</span>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase">Intitulé du Poste</label>
                            <span class="font-bold text-slate-700">{{ currentUser.position || 'N/A' }}</span>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase">Type de Contrat</label>
                            <span class="font-bold text-slate-700">{{ currentUser.contract_type || 'Indéfini' }}</span>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase">Date d'embauche</label>
                            <span class="font-bold text-slate-700">{{ formatDate(currentUser.hiring_date) }}</span>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-[10px] font-bold text-slate-400 uppercase">Statut</label>
                            <Tag :value="currentUser.is_active ? 'Actif' : 'Inactif'" :severity="currentUser.is_active ? 'success' : 'danger'" class="w-fit" />
                        </div>
                    </div>
                </div>

                <div v-if="currentUser.bio">
                    <span class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 block">Biographie</span>
                    <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100 italic text-slate-600 text-sm leading-relaxed">
                        {{ currentUser.bio }}
                    </div>
                </div>

                <div v-if="currentUser.linkedin_url" class="flex items-center gap-4 p-5 bg-blue-50/30 rounded-3xl border border-blue-100">
                    <div class="w-10 h-10 bg-blue-600 text-white rounded-xl flex items-center justify-center shadow-md">
                        <i class="pi pi-linkedin"></i>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[10px] font-black text-blue-400 uppercase tracking-widest">Réseaux Sociaux</span>
    ...
    <div class="flex justify-end p-6 border-t border-slate-100 mt-6">
        <Button label="Fermer le dossier" icon="pi pi-times" severity="secondary" text @click="viewUserSidebar = false" class="font-bold rounded-xl" />
    </div>
</Dialog>
